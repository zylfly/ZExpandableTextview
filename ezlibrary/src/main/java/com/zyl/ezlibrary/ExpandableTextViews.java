...
import com.zyl.ezlibrary.app.StatusType;
import com.zyl.ezlibrary.model.ExpandableStatusFix;
import com.zyl.ezlibrary.model.FormatData;
 ...
 */
public class ExpandableTextViews extends AppCompatTextView {
...
//    public static final String self_regex = "\\[([\\w\\p{InCJKUnifiedIdeographs}-]*)]\\([\\w\\p{InCJKUnifiedIdeographs}-]*\\)";
    ...
    public ExpandableTextViews(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, -1);
    }

    public ExpandableTextViews(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            ...
            isUserUrl = a.getBoolean(R.styleable.ExpandableTextView_ep_need_url, false);
        ...
        if (mNeedLink) {
            ...
            while (matcher.find()) {
                ...
                if (isUserUrl) {
                    ...
                    datas.add(new FormatData.PositionData(newResult.length(), newResult.length() + 1 + content.toString().substring(start, end).length(), matcher.group(), LinkType.LINK_TYPE));
                    newResult.append(" " + content.toString().substring(start, end) + " ");
                }
                temp = end;
            }
        }
        ...
        public boolean onTouchEvent(TextView widget,
            ...
            if (action == MotionEvent.ACTION_UP ||
                ...
                if (link.length != 0) {
                    ...
                    if (widget instanceof ExpandableTextViews) {
                        ((ExpandableTextViews) widget).linkHit = true;
                    }
                    return true;
                } else {
